...
comptime tile = col_major[4, 4,1]()
    # Define a 2x5 tiler
comptime tiler = row_major[4, 5,1]()
...
def main() raises:
    ...
    print(tensor.flat_rank)
    # x = tensor.to_layout_tensor()
    # x = tensor.to_layout_tensor()
    print(leg_tensor[0,0,0])
    print(leg_tensor[0,1,0])
    print(leg_tensor[1,0,0])
    print(leg_tensor[1,1,0])


    print(tensor.load(coord[DType.int32]((0,0,0)) )[0])
    print(tensor.load(coord[0,1,0]()))
    print(tensor.load(coord[1,0,0]()))
    print(tensor.load(coord[1,1,0]()))


    print(tensor[0,0 ,0,0 ,0,0])
    print(tensor[0,0 ,1,0 ,0,0])
    print(tensor[1,0 ,0,0 ,0,0])
    print(tensor[1,0 ,1,0 ,0,0])