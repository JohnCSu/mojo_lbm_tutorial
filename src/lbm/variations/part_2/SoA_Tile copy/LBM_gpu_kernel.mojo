...
def LBM_kernel[ float_dtype:DType,D:Int,Q:Int,
    ...
    if (index[0] < grid_shape[0]) and (index[1] < grid_shape[1]) and (index[2] < grid_shape[2]): # Basic Guard
        ...
        @always_inline
        def vector_sum[width:Int](i:Int) {read f_new, mut rho, read x,read y}:
        ...
        #     f_new[q] = f_new[q] -  inv_tau*(f_new[q]- f_eq)

        # @always_inline
        # def load_f[width:Int](i:Int) {read f_new,var f_out,read x,read,y,read z}:
        #     f_ptr = f_new.unsafe_ptr()
        #     if i  <= len(f_new): # Ensure the load is within bounds
        #         f_out.store[width](coord[DType.uint32]((x,y,z,i)),value = f_ptr.load[width](i))
            
        # vectorize[simd_width](len(f_new),load_f)
        comptime for q in range(Q):
            f_eq = SRT(weights[q],rho,velocity,u_dot_u,float_directions[q])            
