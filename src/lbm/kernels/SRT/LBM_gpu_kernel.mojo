...
from src.lbm import LBM_Grid,LBMConfig
...
def SRT[dtype:DType,D:Int,//,DDF_shift:Bool = False](weight:Scalar[dtype],density:Scalar[dtype],velocity:Vector[dtype,D],u_dot_u:Scalar[dtype],direction:Vector[dtype,D]) -> Scalar[dtype]:
    ...
    comptime if DDF_shift:
        return weight*density*(1 + 3.*ei_dot_u + 4.5*ei_dot_u*ei_dot_u - 1.5*u_dot_u) +weight*(density - 1)
    else:
...
@always_inline
@parameter
def get_DDF_shift_constant[
    float_dtype:DType,Q:Int,D:Int
    ]
    (float_directions:InlineArray[Vector[float_dtype,D],Q],weights:Vector[float_dtype,Q]) 
    -> Vector[float_dtype,D]:
    const_out = Vector[float_dtype,D]()
    comptime for q in range(Q):
        const_out += weights[q]*float_directions[q]
    return const_out
