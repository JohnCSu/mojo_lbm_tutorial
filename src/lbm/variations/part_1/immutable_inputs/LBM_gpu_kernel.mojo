...
def LBM_kernel[ float_dtype:DType,D:Int,Q:Int,
    ...
    comptime f_in_as_lt = LayoutTensor[float_dtype,Flayout.to_layout(),ImmutAnyOrigin]
    
    comptime flag_as_lt = LayoutTensor[DType.uint8,Flaglayout.to_layout(),ImmutAnyOrigin]
    comptime bc_as_lt = LayoutTensor[float_dtype,BClayout.to_layout(),ImmutAnyOrigin]
    comptime f_out_as_lt = LayoutTensor[float_dtype,Flayout.to_layout(),MutAnyOrigin]
    
    comptime assert f_in_as_lt.element_size == 1 and flag_as_lt.element_size == 1 and bc_as_lt.element_size == 1
    
    f_in_lt = f_in_as_lt(f_in.ptr)
    f_out_lt = f_out_as_lt(f_out.ptr)
